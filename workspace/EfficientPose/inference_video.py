...
def main():
    ...
    class_to_3d_bboxes_hook = {class_idx: name_to_3d_bboxes[name] for class_idx, name in class_to_name_hook.items()} 
    ...
    while video.isOpened():
        ...
        input_list_hook, scale_hook = preprocess(image, image_size_hook, camera_matrix, translation_scale_norm)
        
        #predict
        boxes, scores, labels, rotations, translations = model.predict_on_batch(input_list)
        boxes_hook, scores_hook, labels_hook, rotations_hook, translations_hook = model_hook.predict_on_batch(input_list_hook)
        
        #postprocessing
        boxes, scores, labels, rotations, translations = postprocess(boxes, scores, labels, rotations, translations, scale, score_threshold)
        boxes_hook, scores_hook, labels_hook, rotations_hook, translations_hook = postprocess(boxes_hook, scores_hook, labels_hook, rotations_hook, translations_hook, scale_hook, score_threshold)
